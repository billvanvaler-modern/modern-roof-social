...
function getNextScheduledTime(pillar) {
  ...
  for (let daysAhead = 1; daysAhead <= 14; daysAhead++) {
    const candidateTz = new Date(nowInTz);
    candidateTz.setDate(nowInTz.getDate() + daysAhead);
    const dayOfWeek = candidateTz.getDay();
    ...
    // Build the date string in local time and convert to UTC ISO
    const year = candidateTz.getFullYear();
    const month = String(candidateTz.getMonth() + 1).padStart(2, '0');
    const day = String(candidateTz.getDate()).padStart(2, '0');
    const localStr = `${year}-${month}-${day}T${String(hours).padStart(2,'0')}:${String(minutes).padStart(2,'0')}:00`;

    // Parse as local timezone using Intl
    const utcDate = new Date(new Date(localStr).toLocaleString('en-US', { timeZone: 'UTC' }));
    const offsetMs = new Date(localStr).getTime() - new Date(new Date(localStr).toLocaleString('en-US', { timeZone: tz })).getTime();
    const finalDate = new Date(new Date(localStr).getTime() + offsetMs);

    return finalDate.toISOString();
  }

  // Fallback: tomorrow at 11am local time
  const fallback = new Date(new Date().toLocaleString('en-US', { timeZone: tz }));
