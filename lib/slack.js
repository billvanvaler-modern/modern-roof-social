...
async function handleMessage({ text, files, channel, ts, userId }) {
  ...
  try {
    // Download any attached files — up to 5 (Claude's max)
    if (files && files.length) {
      const fileCount = Math.min(files.length, 5);
      await sendMessage(channel, `Got it${files.length > 5 ? ` (using first 5 of ${files.length} photos)` : ` (${fileCount} photo${fileCount > 1 ? 's' : ''})` }! Generating posts... give me 30 seconds.`);
