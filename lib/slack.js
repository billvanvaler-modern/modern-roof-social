...
async function handleMessage({ text, files, channel, ts, userId }) {
  ...
  try {
    // Download any attached files — limit to 2 for Vercel timeout
    const maxFiles = 2;
    if (files && files.length) {
      const fileCount = Math.min(files.length, maxFiles);
      await sendMessage(channel, `Got it${files.length > maxFiles ? ` (using first ${maxFiles} of ${files.length} photos)` : ''}! Generating posts... give me 20 seconds.`);
